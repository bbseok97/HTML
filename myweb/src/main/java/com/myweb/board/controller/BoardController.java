...
public class BoardController extends HttpServlet {
	...
	//	2. 컨텍스트 패스를 제거 /board/list.board 요청으로
	//		들어오면 board_list.jsp 화면으로 이동
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if(command.equals("/board/list.board")) {	//글 목록
		...
		}else if(command.equals("/board/content.board")) {	//	글 목록 페이지
			...
			/*
			 * 	1. UpdateServiceImp1을 생성
			 * 	2. 서비스 영역에서 num, title, content을 받아서 update() 메서드를 실행
			 * 	3. DAO update() 에서는 update 구문으로 데이터를 수정
			 * 	4. 페이지 이름을 상세보기 화면으로 연결 (단, 필요한 값을 전달해야 함)
			 * 
